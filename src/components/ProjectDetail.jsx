...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    // Try to get projects from localStorage first, then fallback to import
    const storedProjects = JSON.parse(localStorage.getItem("projects")) || [];
    let selectedProject = storedProjects.find((p) => String(p.id) === id);
    
    // If not found in localStorage, try to import from data file
    if (!selectedProject) {
      import('../data/projects.json').then(module => {
        const projectsData = module.default;
        const foundProject = projectsData.find((p) => String(p.id) === id);
        if (foundProject) {
          const enhancedProject = {
            ...foundProject,
            Features: foundProject.KeyFeatures || getProjectFeatures(foundProject.id),
            TechStack: foundProject.TechStack || [],
            Github: foundProject.Link || 'https://github.com/EshwarAkhilesh25',
          };
      ...
      const enhancedProject = {
        ...selectedProject,
        Features: selectedProject.KeyFeatures || getProjectFeatures(selectedProject.id),
        TechStack: selectedProject.TechStack || [],
        Github: selectedProject.Link || 'https://github.com/EshwarAkhilesh25',
      };
      setProject(enhancedProject);
    }
  }, [id]);

  // Function to get detailed features for each project
  const getProjectFeatures = (projectId) => {
    const featuresMap = {
      "1": [
        "Implemented 3D volumetric data processing for CT scan analysis",
        "Achieved 96.7% recall rate in lung nodule detection",
        "Built preprocessing pipeline with normalization and augmentation",
        "Created comprehensive evaluation metrics and visualizations",
        "Optimized model performance for clinical deployment"
      ],
      "2": [
        "Implemented Deformable DETR transformer architecture",
        "Enhanced fracture detection accuracy in cervical spine CT scans",
        "Optimized deformable attention mechanisms for better localization",
        "Built comprehensive evaluation pipeline with medical imaging standards",
        "Achieved superior performance compared to baseline methods"
      ],
      "3": [
        "Designed Abstract Syntax Tree (AST) structure for complex rule logic",
        "Built RESTful APIs for rule creation, evaluation, and combination",
        "Implemented MongoDB integration for persistent rule storage",
        "Created rule optimization algorithms to reduce redundant conditions",
        "Developed intuitive frontend for rule management and testing"
      ],
      "4": [
        "Built real-time weather data fetching for multiple Indian cities",
        "Implemented interactive charts using Chart.js for data visualization",
        "Created automated data updates with cron job scheduling",
        "Developed threshold-based weather alert system",
        "Designed responsive UI with Next.js and Tailwind CSS"
      ],
      "5": [
        "Created responsive design with mobile-first approach",
        "Implemented smooth animations using AOS library",
  ...
  navigate('/');
            ...
            <div className="flex items-center space-x-1 md:space-x-2 text-sm md:text-base text-white/50 min-w-0">
              <span>Projects</span>
              <ChevronRight className="w-3 h-3 md:w-4 md:h-4" />
              <span className="text-white/90 truncate max-w-[150px] sm:max-w-none">{project.Title}</span>
            </div>
          </div>
            ...
            <div className="space-y-6 md:space-y-10 animate-slideInLeft">
              ...
              <div className="flex flex-wrap gap-3 md:gap-4">
                ...
                <a
                  ...
                  onClick={(e) => !handleGithubClick(project.Link) && e.preventDefault()}
                >
